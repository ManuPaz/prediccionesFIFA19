...
* **plots_results**. Para visualizar las predicciones: prediccion vs valor real usando gráficos de dispersión o histogramas 2d.
...
* **plots/**. Se guardan los plots con las predicciones vs datos reales sobre el total de los datos y solo sobre los datos de validación para cada modelo, en las carpetas
  ...
  * **regresion_histogramas_2d/**. Histogtramas 2D para regresión para ver lo que ocurre en las zonas de mayor densidad (especialmente para Salario).
...
En el directorio <b>mejoresModelos</b> se guerda el objeto con el modelo seleccionado para cada variable.
...
Los modelos  se encapsulan en un objeto definodo en <b>src/functions/machine_learning</b> que permite calcular las metricas, optimizar parametros,
...
En los mejores modelos incluyendo los jugadores sin equipo empeora el modelo, y no se consigue mejorar ingluyendo una nueva variable binaria que indique si el jugador tiene equipo o no.
...
|-----------------------|------------------|-----------------------|-----------------|----------------------|
| SVR  (kernel rbf)     |0.194             |0.24                   |0.248            |0.305                 |
| regresión lasso       |0.291             |0.291                  |0.371            |0.37                  |
| regresión ridge       |0.291             |0.292                  |0.37             |0.37                  |
| regresión linear      |0.291             |0.292                  |0.37             |0.37                  |
| k_neighbors           |0.327             |0.359                  |0.41             |0.457                 |
| random_forest         |0.121             |0.234                  |0.132            |0.297                 |
| gradient_boosting     |0.06              |0.23                   |0.06             |0.288                 |
| linear SVR            |0.283             |0.285                  |0.399            |0.399                 |
| regresión elastic_net |0.291             |0.291                  |0.371            |0.37                  |
...
En el caso de Ada Boosting se obtuvieron los peores resultados, no solo en esta variable sino en todas, porque se utilizó el modelo de sklearn 
que utiliza por defecto árboles de profundidad 1 como regresor base (es decir, como los regresores que se combinan para generar las predicciones) y en este caso no era 
...
A continuación se muestran los gráficos generados para el modelo que obtuvo los mejores resultados:
...
<h5> Gráfico de dispersión con los datos de validación para Valor de mercado infereriore a 300000 euros </h5>
...
| modelo            |SMAPE en train set|SMAPE en validation set|MAPE en train set|MAPE en validation set|
|-------------------|------------------|-----------------------|-----------------|----------------------|
| SVR  (kernel rbf) |0.351             |0.392                  |0.376            |0.432                 |
| regresión ridge   |0.45              |0.44                   |0.516            |0.512                 |
| regresión lasso   |0.45              |0.44                   |0.516            |0.511                 |
| elastic_net       |0.45              |0.44                   |0.516            |0.511                 |
| random_forest     |0.318             |0.394                  |0.335            |0.451                 |
| gradient_boosting |0.119             |0.4                    |0.119            |0.464                 |
| k_neighbors       |0.429             |0.433                  |0.468            |0.485                 |
| linear SVR        |0.447             |0.44                   |0.498            |0.497                 |
| regsión linear    |0.45              |0.44                   |0.516            |0.512                 |
...
En el validation set tiene un comportamiento similar a train set, pero hay que ver el histrograma 2d con las densidades para los valores más 
bajos de salario.
...
En este último gráfico se aprecia que las predicciones en general son malas excepto para los jugaores de sueldo inferior a 5000 o como mucho hasta 7500. 
  (hay 6787 con sueldo inferior a 5000 de los 13334 que había al quitar los de salario 1000 a los totales). Esto explica porque el SMAPE es de 0.39.
...
<h3>Clasificación </h3>
Igual que para regresión, se muestran las métricas  obtenidos para predecir cada variable (la posición con diferentes agrupamientos).
...
el lado e hizo que mejorase el SMAPE (pasó de ser 0.51 a 0.61 en el mejor modelo), y se utilizó porque se observó en la matriz de confusión que se cometían muchos errores 
acertando la posición pero fallando en el lado.

<h4>Posición sin agrupamiento </h4>

| modelo              |accuracy en train set|accuracy en validation set|
|---------------------|---------------------|--------------------------|
| random_forest       |0.815                |0.57                      |
| gradient_boosting   |0.98                 |0.55                      |
| ada_boosting        |0.395                |0.391                     |
| linear_SVC          |0.602                |0.593                     |
| logistic            |0.607                |0.598                     |
| k_neighbors         |0.579                |0.557                     |
| lda                 |0.578                |0.574                     |
| SVC (kernel linear) |0.625                |0.617                     |

<h4>Posición eliminado en lado </h4>

| modelo              |accuracy en train set|accuracy en validation set|
|---------------------|---------------------|--------------------------|
| k_neighbors         |0.721                |0.705                     |
| random_forest       |0.999                |0.722                     |
| gradient_boosting   |0.966                |0.72                      |
| ada_boosting        |0.565                |0.565                     |
| linear_SVC          |0.734                |0.738                     |
| logistic            |0.744                |0.74                      |
| lda                 |0.718                |0.711                     |
| SVC (kernel linear) |0.759                |0.752                     |

<h4>Posición agrupada en Portero, Medio, Defensa y Delantero </h4>

| modelo              |accuracy en train set|accuracy en validation set|
|---------------------|---------------------|--------------------------|
| random_forest       |0.859                |0.859                     |
| gradient_boosting   |0.86                 |0.86                      |
| ada_boosting        |0.58                 |0.58                      |
| linear_SVC          |0.865                |0.865                     |
| logistic            |0.866                |0.866                     |
| k_neighbors         |0.849                |0.849                     |
| lda                 |0.853                |0.853                     |
| SVC (kernel linear) |0.871                |0.871                     |
...
y para Ada Boosting y K neighbors se consiguen peores resultados también por los mismos motivos que en el caso de regresión.
<br><br>
...
con central (CB) (todos se predicen como central) lo cual es lógico porque tienen las mismas caractarísticas.
...
<h5> Variable: Posición eliminado el lado. Usando modelo SVC con kernel lineal </h5>
...
en la precisión para esta clase y el recall para las otras, ya que cuando hay una clase 
dominante se puede llegar a obtener buena accuracy prediciendo mucho esa clase.<br><br> En este caso sin embargo vemos que la precisión de CB es alta, 
