...
def clustering_kmeans(df_transformed, position, nombre_dir_plots):
    ...
    # optimizacion de k
    if buscar_k:
        costes_finales, centroides_finales, metricas = functions_clustering.optimizar_k(k_min, k_max, df_transformed,
                                                                                        n_init,
                                                                                        max_iter, position)

        functions_clustering.k_vs_metric(k_min, k_max, costes_finales,
                                         nombre_archivo=nombre_dir_plots + "/optimizacion/" + "elbow_method_" + str(
                                             n_comps) + "comps+_metrica_coste.jpg", title="Variacion del coste")
    ...
    model, coste, centroides = functions_clustering.train_k_means(k_elegido, df_transformed, inicio="random",
                                                                  n_init=n_init,
                                                                  max_iter=max_iter)
    ...
    if k_elegido in num_clases:
        ...
        cm = aux
        plots.plot_heat_map(cm,
                            nombre_dir_plots + "/heatmaps/" + "heat_map_k" + str(k_elegido) + "_ncomps" + str(n_comps),
...
def clustering_dbscan(df_trasnformed, position, nombre_dir_plots):
    optimizar = False
    if optimizar:
        resultados = functions_clustering.optimize_dbscan(df_trasnformed, position, eps=np.arange(0.1, 0.7, 0.1),
                                                          min_samples=np.arange(10, 300, 30))
    ...
    cm = pd.DataFrame(index=df_transformed.position.unique(), columns=df_transformed.cluster.unique())
    for i in cm.index:
        for k in cm.columns:
            cm.loc[i, k] = df_trasnformed.loc[(df_trasnformed.position == i) & (df_trasnformed.cluster == k)].count()[0]
    plots.plot_heat_map(cm, nombre_dir_plots + "/heatmaps/" + "_ncomps" + str(n_comps),
                        "{} vs cluster".format(feature), "Cluster", " {}".format(feature))
